...
def add_box(request):
    return render(request, 'dashboard/add_box.html')


@staff_member_required
def box_admin(request):
    boxes = Box.objects.all().order_by('-shipping_date')
    return render(request, 'dashboard/box_manager.html', {'boxes':boxes})
...
def edit_box(request, box_id):
    ...
    return render(request, 'dashboard/edit_box.html', {'form': form, 'box_id': box_id})
